...
class CDDownloader(QObject):
    ...
    Element fields: word_title, word_gram, word_pro_uk, word_pro_us, word_uk_media, word_us_media, word_image, meanings
    """
    ...
    def get_word_defs(self):
        ...
        for tag_cald4 in word_soup.find_all(name='div', attrs={'class': 'pr dictionary',
                                                               'data-id': ['cald4', 'cbed', 'cacd', 'cald4-us']}):
            for tag_entry in tag_cald4.find_all(name='div', attrs={
                'class': ['pr entry-body__el', 'pr idiom-block', 'entry-body__el clrd js-share-holder']}):
                ...
                if cur_tag:
                    ...
                    if media_file_tag:
                        if media_file_tag['src'] in self.word_media:
                            ...
                            word_to_add.word_uk_media = self.get_tempfile_from_url(
                                self.base_url.rstrip('/') + media_file_tag['src'])
                ...
                if cur_tag:
                    ...
                    if media_file_tag:
                        if media_file_tag['src'] in self.word_media:
                            ...
                            word_to_add.word_us_media = self.get_tempfile_from_url(
                                self.base_url.rstrip('/') + media_file_tag['src'])
                            self.word_media[media_file_tag['src']] = word_to_add.word_uk_media
                ...
                # (PICTURE)'
                for html_l2_tag in tag_entry.find_all(name=['div', 'span'], attrs={
                    'class': ['pos-body', 'idiom-body didiom-body', 'pv-body dpv-body']}):
                    # Looping through words specific definitions - l2_meaning
                    # (def & examples)
                    for html_pos_body in html_l2_tag.find_all(
                            attrs={'class': ['pr dsense', 'pr dsense ', 'sense-body dsense_b']}):
                        ...
                        l2_meaning_examples = []
                        l2_meaning = {}
                        for html_meaning in html_pos_body.find_all(name="div", attrs={
                            'class': ['def-block ddef_block', 'def-block ddef_block ']}):
                            ...
                            if tag_picture:
                                if tag_picture.attrs['src'] in self.word_media:
                                    ...
                                    word_to_add.word_image = self.get_tempfile_from_url(
                                        self.base_url.rstrip('/') + tag_picture.attrs['src'])
                            ...
                            word_to_add.word_specific_gram = self._prettify_string(
                                tag_l2_specific_gram.text if tag_l2_specific_gram else '')
                            ...
                            word_to_add.word_specific = self._prettify_string(
                                tag_l2_specific.text if tag_l2_specific else '')
                    ...
                    for html_pos_body in html_l2_tag.find_all(name='div', attrs={'class': 'pr', 'class': 'dsense',
                                                                                 'class': 'dsense-noh'}):
                        ...
                        l2_meaning_examples = []
                        l2_meaning = {}
                        for html_meaning in html_pos_body.find_all(name="div",
                                                                   attrs={'class': 'def-block', 'class': 'ddef_block'}):
                            tag_l2_meaning = html_meaning.find("div", attrs={'class': 'ddef_h'})
                            if not tag_l2_meaning:
                                continue
                            # Image
    ...
    def get_tempfile_from_url(self, url_in):
        ...
        # here.)
        tfile = tempfile.NamedTemporaryFile(delete=False, prefix='anki_audio_', suffix=self.file_extension)
        tfile.write(data)
        tfile.close()
        return tfile.name
